...
{
    /// <summary>
    /// Mock actor state provider (from ServiceFabric.Mocks)
    /// </summary>
    public class MockActorStateProvider : IActorStateProvider
    {
        ...
        public Task<T> LoadStateAsync<T>(ActorId actorId, string stateName, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            if (actorState.TryGetValue(stateName, out var stateEntry))
            {
                result = (T)stateEntry;
            }
            return Task.FromResult(result);
        ...
        public Task SaveReminderAsync(ActorId actorId, IActorReminder reminder,
            CancellationToken cancellationToken = new CancellationToken())
        {
            var reminderState = CreateReminderState(actorId, reminder);
            _reminders.Delete(actorId, reminderState.Name);
            _reminders.Add(actorId, reminderState);
            return Task.FromResult(reminderState);
        }

        protected virtual IActorReminderState CreateReminderState(ActorId actorId, IActorReminder reminder)
        {
            var utcNow = DateTime.UtcNow;
            var timeOfDay = utcNow.TimeOfDay;

            return new MockActorReminderState(new MockActorReminderData(actorId, reminder, timeOfDay), timeOfDay);
        }
        ...
        public async Task DeleteRemindersAsync(IReadOnlyDictionary<ActorId, IReadOnlyCollection<string>> reminderNames, CancellationToken cancellationToken = default)
        {
